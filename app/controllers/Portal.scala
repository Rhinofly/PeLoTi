...
class Portal(address: String) extends Controller with RequestHandler {
  ...
  def requestResetHandler = Action.async { implicit request =>
    tryWithRecover {
      requestResetForm.bindFromRequest.fold(
        ...
        email => postWS("requestReset", Map("email" -> Seq(email))) {
          response =>
  ...
  private def postWS(path: String, parameters: Map[String, Seq[String]])(function: Response => SimpleResult): Future[SimpleResult] =
